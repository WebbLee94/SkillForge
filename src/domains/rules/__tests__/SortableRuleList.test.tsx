...
describe('SortableRuleList', () => {
  it('renders empty state when rules array is empty', () => {
    render(<SortableRuleList rules={[]} onRemove={vi.fn()} onToggle={vi.fn()} />);
    expect(screen.getByText('dragHint')).toBeDefined();
  });

  it('renders rule items with names', () => {
    render(<SortableRuleList rules={mockRules} onRemove={vi.fn()} onToggle={vi.fn()} />);
  ...
  it('calls onToggle when toggle button is clicked', () => {
    ...
    render(<SortableRuleList rules={mockRules} onRemove={vi.fn()} onToggle={onToggle} />);
  ...
  it('calls onRemove when X button is clicked', () => {
    ...
    render(<SortableRuleList rules={mockRules} onRemove={onRemove} onToggle={vi.fn()} />);
  ...
  it('does not show remove buttons when disabled', () => {
    render(<SortableRuleList rules={mockRules} onRemove={vi.fn()} onToggle={vi.fn()} disabled={true} />);
  ...
  it('uses rule_id fallback when rule_name is falsy', () => {
    ...
    render(<SortableRuleList rules={[ruleNoName]} onRemove={vi.fn()} onToggle={vi.fn()} />);
