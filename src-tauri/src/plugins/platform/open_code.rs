...
/// Manages skills via symlinks in ~/.config/opencode/skills/ (global) and .opencode/skills/ (project).
...
impl PlatformPlugin for OpenCodeAdapter {
    ...
    fn detect(&self) -> Result<Vec<PlatformInstance>, AppError> {
        ...
        if global_dir.exists() || expand_home("~/.config/opencode").exists() {
            instances.push(PlatformInstance {
                platform_id: "opencode".to_string(),
                platform_name: "OpenCode".to_string(),
    ...
    fn install(&self, skill: &Skill, instance: &PlatformInstance) -> Result<(), AppError> {
        ...
        #[cfg(not(unix))]
        {
            crate::plugins::platform::claude_code::copy_dir_recursive(&source, &target)?;
        }
    ...
    fn status(&self, skill_id: &str, instance: &PlatformInstance) -> Result<SkillPlatformStatus, AppError> {
        ...
        let is_valid = target.is_symlink()
            && target
                .read_link()
        ...
        Ok(SkillPlatformStatus {
            installed: is_valid || (target.exists() && target.is_dir()),
    ...
    fn default_paths(&self) -> PlatformPaths {
        PlatformPaths {
            global_skills_dir: "~/.config/opencode/skills".to_string(),
            project_skills_pattern: "{project}/.opencode/skills".to_string(),
            global_rules_dir: Some("~/.config/opencode/rules".to_string()),
            project_rules_pattern: Some(".opencode/rules".to_string()),
        }
    }
}
