...
/// Manages skills and rules via symlinks:
/// - Skills: ~/.cursor/skills/ (global), .cursor/skills/ (project)
/// - Rules:  ~/.cursor/rules/ (global), .cursor/rules/ (project)
...
impl PlatformPlugin for CursorAdapter {
    ...
    fn detect(&self) -> Result<Vec<PlatformInstance>, AppError> {
        ...
        if global_dir.exists() || expand_home("~/.cursor").exists() {
    ...
    fn default_paths(&self) -> PlatformPaths {
        PlatformPaths {
            ...
            project_skills_pattern: "{project}/.cursor/skills".to_string(),
            global_rules_dir: Some("~/.cursor/rules".to_string()),
            project_rules_pattern: Some("{project}/.cursor/rules".to_string()),
        }
    }
}
