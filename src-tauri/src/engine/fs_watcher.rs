...
enum MuteVerdict { Live, SelfWrite, Foreign }

fn classify_mute(now_ms: u64, deadline_ms: u64, roots: &[PathBuf], event_paths: &[PathBuf]) -> MuteVerdict {
    if !muted_at(now_ms, deadline_ms) { return MuteVerdict::Live; }
    if !event_paths.is_empty() && event_paths.iter().all(|p| roots.iter().any(|r| p.starts_with(r))) {
...
pub fn mute_self_writes(target: &Path) {
    ...
    if !muted_at(now, state.deadline_ms) { state.roots.clear(); }
    state.deadline_ms = state.deadline_ms.max(now + SELF_WRITE_MUTE.as_millis() as u64);
...
pub fn collect_watch_paths(_conn: &rusqlite::Connection) -> Vec<PathBuf> {
    ...
    let platforms = [
        ".claude", ".opencode", ".cursor", ".trae", ".trae-cn",
        ".codebuddy", ".codebuddy-cn", ".codex", ".windsurf",
...
// ── Watch set sync ──
fn sync_watch_set(watcher: &mut RecommendedWatcher, watched: &mut HashSet<PathBuf>, desired: &HashSet<PathBuf>) -> bool {
    ...
    for path in desired {
        if watched.contains(path) { continue; }
...
fn should_emit(event: &Event) -> bool {
    if event.paths.is_empty() { return false; }
    event.paths.iter().any(|p| !is_in_git_dir(p))
}

fn is_in_git_dir(path: &Path) -> bool {
    path.components().any(|c| c.as_os_str() == std::ffi::OsStr::new(".git"))
}
...
fn is_skill_related(path: &Path) -> bool {
    ...
    if name == "__pycache__" || name.ends_with(".pyc") { return false; }
    if name.starts_with(".git") { return false; }
...
pub fn start_file_watcher<R: Runtime>(app: AppHandle<R>, watch_paths: Vec<PathBuf>) {
    ...
    std::thread::spawn(move || {
        ...
        let mut watcher = match RecommendedWatcher::new(
            ...
            Ok(w) => w,
            Err(e) => { log::error!("无法创建文件监控器: {e}"); return; }
        };
        ...
        loop {
            if last_sync.elapsed() >= WATCH_RESCAN_INTERVAL {
                ...
                match decide_emit(changed, classify_event_paths(&[]), last_emit.elapsed() < WATCH_EMIT_DEBOUNCE) {
            ...
            match rx.recv_timeout(Duration::from_millis(500)) {
                Ok(Ok(event)) => {
                    ...
                    for path in &event.paths {
                        if !is_skill_related(path) { continue; }
                    ...
                    if !roots.is_empty() {
                        ...
                        for root in roots {
                            ...
                            if resolve_path_inline(&root_str).is_none() { continue; }
                            if kind == "MODIFIED" && events.iter().any(|e| e.path == root_str && e.event_type == "NEW") {
                                continue;
                            }
                            if events.iter().any(|e| e.path == root_str && e.event_type == kind) {
                                continue;
                            }
                            let id = {
                    ...
                    match decide_emit(should_emit(&event), classify_event_paths(&event.paths), last_emit.elapsed() < WATCH_EMIT_DEBOUNCE) {
                        EmitAction::Emit => emit_now(&mut last_emit, &mut pending_emit),
                        EmitAction::Defer => pending_emit = true,
                        EmitAction::Skip => {}
                    }
...
pub fn get_pending_events() -> Vec<(u64, String, String)> {
    let events = PENDING_EVENTS.get().map(|m| m.lock().unwrap().clone()).unwrap_or_default();
    events.into_iter().map(|e| (e.id, e.event_type, e.path)).collect()
}
...
pub fn build_watch_paths(enabled_platform_ids: &[String]) -> Vec<PathBuf> {
    ...
    let mut paths = Vec::new();
    for (id, dir) in all {
        if !enabled_platform_ids.iter().any(|e| e == id) { continue; }
...
mod tests {
    ...
    fn classify_mute_path_scoped() {
        ...
        assert_eq!(classify_mute(5, 10, &roots, &inside), MuteVerdict::SelfWrite);
    ...
    fn decide_emit_routing() {
        ...
        assert_eq!(decide_emit(true, MuteVerdict::SelfWrite, false), EmitAction::Skip);
        // Foreign → always Defer
        assert_eq!(decide_emit(true, MuteVerdict::Foreign, false), EmitAction::Defer);
        assert_eq!(decide_emit(true, MuteVerdict::Foreign, true), EmitAction::Defer);
        // Live + not debounced → Emit
        assert_eq!(decide_emit(true, MuteVerdict::Live, false), EmitAction::Emit);
        // Live + debounced → Skip
        assert_eq!(decide_emit(true, MuteVerdict::Live, true), EmitAction::Skip);
        // Irrelevant → always Skip
        assert_eq!(decide_emit(false, MuteVerdict::Foreign, false), EmitAction::Skip);
    }

    #[test]
    fn build_watch_paths_filters_by_enabled_platforms() {
        let all_ids: Vec<String> = [
            "claude-code", "opencode", "cursor", "trae", "trae-cn",
            "codebuddy", "codebuddy-cn", "codex", "windsurf",
            "hermes", "openclaw", "antigravity",
        ].iter().map(|s| s.to_string()).collect();
        let paths = build_watch_paths(&all_ids);
        assert!(paths.len() >= 36, "expected >=36 paths, got {}", paths.len());
